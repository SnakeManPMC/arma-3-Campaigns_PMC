...
// trick! when player reaches objective 5 we activate more bad guys BEHIND HIS BACK for the way home HAHA
[] spawn
{
	waitUntil
	{
		sleep 10;
		(triggerActivated pmc_5);
	};
	_grp = [getPos pmc_location_1] call PMC_Create_CUP_NAPA_Random_Infantry;
	[(getPos pmc_location_1), _grp, 500] execVM "PMC\PMC_defendLocation.sqf";

	_grp = [getPos pmc_location_2] call PMC_Create_CUP_NAPA_Random_Infantry;
	[(getPos pmc_location_2), _grp, 500] execVM "PMC\PMC_defendLocation.sqf";

	_grp = [getPos pmc_location_3] call PMC_Create_CUP_NAPA_Random_Infantry;
	[(getPos pmc_location_3), _grp, 500] execVM "PMC\PMC_defendLocation.sqf";
};
