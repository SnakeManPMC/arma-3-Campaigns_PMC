...
"_targetNum","_targetpoint","_PMC_t","_MaxObjectives","_exit"
];
        ...
        switch (_name) do {
                ...
                case "PMC_Objectives":
                {
                        if !(PMC_Objectives_Created) then { 
                                {_ call _PMC_CreateObjective} forEach  (_this select 1);
...
if (isServer) then {
        ...
        if (_ObjCount == 0) then
        {
                if (pmcdb) then { player sideChat "_objcount was zero..."; };
                _ObjCount = 1;
        };
        ...
        {_x call _PMC_CreateObjective} forEach PMC_Objectives;
        PMC_Objectives_Created=true;
