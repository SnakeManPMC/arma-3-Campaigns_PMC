...
titletext["Special Operations Forces deep behind enemy lines.","plain down",2];

// assault1 leader
_camera camSetTarget leader assault1;
_camera camSetRelPos [0,3,1.3];
_camera camCommit 10;
waitUntil { (camCommitted _camera); };
sleep 3;

titletext["They are HQ's eyes and ears on the battlefield, the extremely\nimportant intelligence gathering tool.","plain down",2];
...
titletext["These guys are just itching to get some real action,\nbut that has to wait, for when the time is right...","plain down",2];
